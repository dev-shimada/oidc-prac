...
// IdTokenHeader represents the header of an ID Token in JWT format.
...
// JWT represents a JSON Web Token (JWT) structure.
type JWT struct {
...
// IdTokenMaker is an interface for creating ID Tokens.
type IdTokenMaker[T JWS | JWE] interface {
	Make(T) (string, error)
}

// JWS represents a JSON Web Signature (JWS) structure.
type JWS struct {
